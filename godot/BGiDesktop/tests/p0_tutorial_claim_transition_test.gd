...
func _run() -> void:
	...
	_expect(bool(tutorial.complete_resolved_current_task("starter_01")["is_completed"]), "a persisted fixed result must advance tutorial progression without waiting for collection")
	_expect(str(progression.get_current_task()["id"]) == "starter_02", "completion must advance to the next fixed tutorial task")
	_expect(_status_for(game_state.get_crew(), "crew_01") == GameStateScript.CrewStatus.AVAILABLE, "completion must restore crew availability before collection")
	_expect(assignment_state.get_assigned_crew_ids("starter_01").is_empty(), "completion must clear the resolved task assignment")
	...
	_expect(str(progression.get_current_task()["id"]) == "starter_02", "claim must not regress the already-advanced tutorial task")
