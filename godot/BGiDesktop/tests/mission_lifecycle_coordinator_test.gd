...
func _run() -> void:
	...
	_expect(_status_for(game_state.get_crew(), "crew_01") == GameStateScript.CrewStatus.AVAILABLE, "saved pending-claim completion must release crew before claim")
	_expect(assignment_state.get_assigned_crew_ids("starter_01").is_empty(), "pending-claim completion must release its assignment")
	lifecycle.rollback_completion_after_save_failure("starter_01")
	_expect(_status_for(game_state.get_crew(), "crew_01") == GameStateScript.ASSIGNED_STATUS, "unsaved completion rollback must restore assigned crew")
	_expect(lifecycle._locked_results_by_mission_run_id.is_empty(), "unsaved completion rollback must discard the unpersisted result")
	resolved_result = lifecycle.resolve_completed_result("starter_01", 105)
	_expect(bool(resolved_result["is_resolved"]) and bool(resolved_result["did_resolve"]), "completion must resolve again after its save rollback")
	_expect(_status_for(game_state.get_crew(), "crew_01") == GameStateScript.CrewStatus.AVAILABLE, "re-resolved pending claim must release crew again")
	...
	lifecycle.rollback_claim_after_save_failure("starter_01", Dictionary(first_claim["receipt"]))
	_expect(_status_for(game_state.get_crew(), "crew_01") == GameStateScript.CrewStatus.AVAILABLE, "claim-save rollback must keep completed crew available")
	_expect(not bool(lifecycle.get_claim_receipt(str(first_claim["receipt"]["mission_run_id"]))["is_found"]), "claim-save rollback must remove the unpersisted receipt")
	var restored_pending_run: Dictionary = Dictionary(lifecycle.get_persisted_runs()["mission_runs_by_id"][str(first_claim["receipt"]["mission_run_id"])])
	_expect(str(restored_pending_run["run_state"]) == "completed_pending_claim", "claim-save rollback must restore pending-claim state")
	var retry_claim: Dictionary = lifecycle.claim_completed_result("starter_01", 106)
	_expect(bool(retry_claim["is_claimed"]) and bool(retry_claim["did_claim"]), "pending claim must remain retryable after failed save")

	var repeated_claim: Dictionary = lifecycle.claim_completed_result("starter_01", 107)
	...
	_expect(Dictionary(repeated_claim["receipt"]) == Dictionary(retry_claim["receipt"]), "repeated claim must preserve the original receipt")
