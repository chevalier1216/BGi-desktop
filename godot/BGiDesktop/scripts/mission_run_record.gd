...
static func from_data(data: Dictionary) -> Dictionary:
	...
	return create(str(data.get("mission_run_id", "")), str(data.get("mission_template_id", "")), crew_ids, int(data.get("started_at_seconds", -1)), int(data.get("due_at_seconds", -1)), str(data.get("run_state", "")), Dictionary(data.get("result_snapshot", {})), str(data.get("claim_receipt_id", "")))

static func _is_valid_result(result_snapshot: Dictionary, mission_run_id: String) -> bool:
	var descriptors: Dictionary = ClaimEffectDescriptorScript.validate_all(result_snapshot.get("claim_effect_descriptors", null))
	return str(result_snapshot.get("mission_run_id", "")) == mission_run_id and not str(result_snapshot.get("result_id", "")).is_empty() and bool(descriptors["is_valid"])
