...
func _run() -> void:
	...
	_expect(_status_for(failing["game_state"].get_crew(), "crew_01") == GameStateScript.CrewStatus.COMPLETED, "receipt write failure must keep crew unavailable while the result remains claimable")
