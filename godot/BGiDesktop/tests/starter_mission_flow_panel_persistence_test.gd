...
func _run() -> void:
	...
	_expect(first_panel.status_label.text == "等待中：已派遣 3 名小弟", "started task must save its three assigned crew members")
	...
	_expect(completed_panel.status_label.text == "已完成／保底報酬待定", "expired reopened task must save and show its locked result")
	...
	_expect(locked_panel.status_label.text == "已完成／保底報酬待定", "reopened locked result must retain its completed presentation")
	...
	_expect(locked_panel._selected_crew_ids == ["crew_01", "crew_02", "crew_03"], "completed restoration must retain its assigned crew ids")
