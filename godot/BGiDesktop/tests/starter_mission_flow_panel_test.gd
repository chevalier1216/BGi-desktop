...
func _run() -> void:
	...
	_expect(panel.guaranteed_reward_label.text.is_empty(), "client-facing task panel must not expose the internal guaranteed-reward placeholder")
	_expect(panel.extra_reward_range_label.text.is_empty(), "client-facing task panel must not expose the internal extra-reward range placeholder")
	_expect(panel.extra_reward_probability_label.text.is_empty(), "client-facing task panel must not expose the internal extra-reward probability placeholder")
	_expect(panel.extra_reward_note_label.text.is_empty(), "client-facing task panel must not expose internal reward-resolution copy")
