...
- 每個轉換以「先保存下一筆權威資料，再改 UI」為原則。任務完成時，結果快照、其中已解析的 `ClaimEffectDescriptor`、`completed_pending_claim` 與人物恢復可用必須一同安全保存；保存失敗時停留在前一個已保存狀態，不釋放人物、不推進教學、不增加地盤效果。
...
| `completed_pending_claim` | 結果與效果描述已安全固定，可由玩家收取。 | 已保存 `MissionRunRecord.result_snapshot` 及其 `ClaimEffectDescriptor`，`run_state=completed_pending_claim`，無 `claim_receipt_id`。 | 原派遣人物已保存為可用，可參與其他任務。 | 可收取卡與固定結果；人物區不得將其標為忙碌。 | lifecycle coordinator 完成結果與人物可用狀態保存後。 |
| `claim_committing` | 玩家已點收取，正在保存收據與已固定效果。 | `ClaimReceipt` 草稿引用已保存的 `ClaimEffectDescriptor`，不重新解析效果。 | 人物維持可用；本交易不再次變更人物可用性。 | 僅顯示收取處理中；保留可收取結果畫面。 | lifecycle coordinator。 |
...
| `active` | `now_seconds >= due_at_seconds`，時計與隊伍仍可驗證。 | 建立／讀回固定結果及其 `ClaimEffectDescriptor`；保存 `result_snapshot`、`run_state=completed_pending_claim` 與原派遣人物恢復可用狀態。 | `completed_pending_claim` | 首次固定後，重啟或重試均回傳同一 `result_id` 及同一效果描述符；人物可再派遣，但成果未入帳。 |
| `completed_pending_claim` | 玩家按收取，尚無 `ClaimReceipt`。 | 保存唯一 `ClaimReceipt`（引用固定描述符）→ 套用已識別效果 → 保存地盤／人物解鎖／教學變化 → 保存 `run_state=claimed`。 | `claimed` | 同一 `mission_run_id` 最多一張收據；人物在此交易前已可用，且不重複套用效果。 |
...
| 固定結果後從目前設定、掉落表或內容對照重算效果 | 會改寫已知地盤、人物或收藏成果。 | 只讀回已保存的 `ClaimEffectDescriptor`；缺少必要描述符時進入 `recovery_hold`。 |
...
| 有完整活動任務，載入時已到期，尚無固定結果。 | 先執行 `active → completed_pending_claim` 的固定結果、效果描述符與人物釋放保存。 | 保存成功後顯示可收取，原派遣人物可用。 | 未保存前顯示收取或釋放人物。 |
| 有固定結果及效果描述符，尚無收據。 | `completed_pending_claim`。 | 顯示同一固定結果與收取按鈕；人物顯示可用，任務卡顯示待收取。 | 重骰、重派同一任務或自動收取。 |
...
4. 以相同 `mission_template_id` 建立兩次不同 `mission_run_id`，兩筆結果、效果描述符與收據不能互相覆寫。
