...
func _run() -> void:
	...
	_expect(str(receipt["result_id"]) == "starter_01:100:105", "receipt must retain the fixed result id")
	...
	var reopened: Dictionary = _create_lifecycle(RECEIPT_PATH)
...
func _create_lifecycle(receipt_path: String) -> Dictionary:
	...
	return {
		...
		"lifecycle": LifecycleScript.new(assignment_coordinator, expired_release_service, snapshot_collection, receipt_store),
	}
