...
func _run() -> void:
	...
	_expect(str(progression.get_current_task()["id"]) == "starter_01", "expiry must not advance tutorial progression")
	_expect(_status_for(game_state.get_crew(), "crew_01") == GameStateScript.CrewStatus.COMPLETED, "expired crew must remain completed pending claim")
	_expect(not assignment_state.get_assigned_crew_ids("starter_01").is_empty(), "expired task must remain assigned pending claim")
	...
	_expect(bool(tutorial.complete_claimed_current_task("starter_01", Dictionary(claim["receipt"]))["is_completed"]), "saved receipt must permit tutorial progression")
	_expect(str(progression.get_current_task()["id"]) == "starter_02", "claim must advance to the next fixed tutorial task")
	_expect(_status_for(game_state.get_crew(), "crew_01") == GameStateScript.CrewStatus.AVAILABLE, "successful claim must restore completed crew availability")
