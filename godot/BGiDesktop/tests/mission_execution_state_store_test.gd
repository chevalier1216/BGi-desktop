...
func _run() -> void:
	...
	var crew_ids_by_task: Dictionary = {"starter_01": ["crew_01", "crew_02", "crew_03"]}
	_expect(bool(store.save(collection, empty_result_state, crew_ids_by_task)["is_saved"]), "unfinished execution state must save with assigned crew ids")
	...
	_expect(restored_pending["crew_ids_by_task"]["starter_01"] == ["crew_01", "crew_02", "crew_03"], "reopened execution must retain its assigned crew ids")
	...
	_expect(bool(store.save(restored_pending["collection"], locked_result_state, restored_pending["crew_ids_by_task"])["is_saved"]), "locked execution result must save with its snapshot and crew ids")
